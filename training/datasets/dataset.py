...
class DirectoryAsZip:
    ...
    def infolist(self):
        return [_FakeZipInfo(str(p.relative_to(self.root))) for p in self._files]
...
def _open_archive(path):
    ...
    if p.is_dir():
        return DirectoryAsZip(p)
    raise FileNotFoundError(f"Neither file nor directory: {path}")
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if annotations_json_path_in_zip is not None:
            with _open_archive(target_zip_path or zip_path) as outer_target_zip:
    ...
    def _load_zips(
        ...
        if worker not in self.zip:
            self.zip[worker] = _open_archive(self.zip_path)
        if worker not in self.target_zip:
            if self.target_zip_path:
                self.target_zip[worker] = _open_archive(self.target_zip_path)
